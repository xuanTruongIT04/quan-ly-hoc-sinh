...
| Phase | Goal | Status |
|-------|------|--------|
| **1 — MVP** | Next.js + mock data (seed `.ts` → localStorage), Vietnamese content, dashboard + thống kê · quản lý học sinh (CRUD, nhập hàng loạt) · điểm danh (có hàng loạt) · tính học phí, deploy Vercel to demo | **We are here** |
| **2 — Phiếu học phí** | Xuất **phiếu học phí** (html2canvas → ảnh) + **VietQR** thanh toán + **nhận xét** tháng | Not started |
| **3 — Full** | **Lịch dạy** + điểm danh giáo viên · **5 theme** (Mặc Định/Đại Dương/Oải Hương/Dâu Tây/Sang Trọng) · **thống kê/biểu đồ** doanh thu (Chart.js) theo ngày/tháng/năm · hoàn thiện UX · bàn giao cho Trang Nhung | Not started |

> ⚠️ Nghiên cứu sâu bản gốc theo từng phase: Phase 2 nghiên cứu kỹ phiếu học phí/VietQR/nhận xét
> (account trial hiện 0 học sinh nên các luồng cần dữ liệu chưa xem được) trước khi code.
